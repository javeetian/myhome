...
# 把一个二进制文件写成 C 数组
function Add-CArray($Sb, [string]$Name, [string]$Path) {
    $bytes = [IO.File]::ReadAllBytes($Path)
    [void]$Sb.AppendLine("const unsigned char $Name[] = {")
    ...
    [void]$Sb.AppendLine('};')
    [void]$Sb.AppendLine("const unsigned int ${Name}_len = $($bytes.Length);")
    [void]$Sb.AppendLine('')
}

# 把设备的 UI 资源编成 const 数组 (ui_assets.c), 随固件编译 —— 不依赖设备文件系统。
# 源: <设备目录>\build\ui.pkg (Studio 生成) + <设备目录>\ui\manifest.json
# 实测这块板子上 dev_manager 的设备都 valid=0 且目录为空, 文件系统取不到 UI 资源。
# UI 包只有 KB 级, 直接编进固件最省事。改完 UI 重跑本脚本即可。
function New-UiAssets([string]$DevDir, [string]$DevName) {
    $pkg = Join-Path $DevDir 'build\ui.pkg'
    $man = Join-Path $DevDir 'ui\manifest.json'
    if (-not (Test-Path -LiteralPath $pkg)) {
        Write-Host "      ${DevName}: 没有 build\ui.pkg, 跳过资源数组"
        return 0
    }
    $sb = New-Object System.Text.StringBuilder
    [void]$sb.AppendLine('/* Generated by sdk/device/deploy_to_sdk.ps1 -- do not edit by hand.')
    [void]$sb.AppendLine(' * UI assets baked into the firmware: build/ui.pkg + ui/manifest.json */')
    [void]$sb.AppendLine('')
    Add-CArray $sb 'k_ui_pkg' $pkg
    if (Test-Path -LiteralPath $man) {
        Add-CArray $sb 'k_ui_manifest' $man
    }
    $outFile = Join-Path $DevDir 'ui_assets.c'
    [IO.File]::WriteAllText($outFile, $sb.ToString(), [Text.ASCIIEncoding]::new())
    $kb = [Math]::Round((Get-Item -LiteralPath $pkg).Length / 1KB, 1)
    Write-Host "      ${DevName}: ui_assets.c ($kb KB → const 数组)"
    return 1
}
...
# 每个设备: 先把 UI 资源编成 ui_assets.c, 再列出 设备根目录 *.c + generated/c/*.c
foreach ($dev in $devices) {
    [void](New-UiAssets -DevDir $dev.FullName -DevName $dev.Name)
    $srcList = New-Object System.Collections.Generic.List[string]
    foreach ($f in (Get-ChildItem -Path $dev.FullName -Filter '*.c' -File | Sort-Object Name)) {
        $srcList.Add("apps/myhome/$($dev.Name)/$($f.Name)")   # device_app.c / ui_assets.c ...
    }
