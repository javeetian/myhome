            ...
            when (connectionResult) {
                is EstablishedConnection ->
                    connectionResult.rxConnection.requestConnectionPriority(
                        priority.code,
                        // PATCH(myhome): 上游是 2, TimeUnit.SECONDS。
                        // 那是 RxAndroidBle 的固定空转延迟（Android 没有连接参数更新的
                        // 完成回调，ConnectionPriorityChangeOperation.getCallback()
                        // 直接返回 Single.timer），而且这 2 秒里该操作一直占着每连接
                        // 的操作队列 —— 紧随其后的 discoverServices() 会被压在后面，
                        // 实测把服务发现从 ~100ms 拖到 1.7-3.4s。
                        // 真正的 GATT 调用在 startOperation() 里已同步发出，这个值
                        // 只是"等成功"的时间；RxAndroidBle 要求 > 0，故取 1ms。
                        // 详见 third_party/reactive_ble_mobile/PATCH.md
                        1,
